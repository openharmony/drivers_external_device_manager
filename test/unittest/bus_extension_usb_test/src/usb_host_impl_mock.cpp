...
UsbHostImplMock::UsbHostImplMock() { }

UsbHostImplMock::~UsbHostImplMock() { }

int32_t UsbHostImplMock::GetRawDescriptor(const UsbDev &dev, std::vector<uint8_t> &descriptor)
{
...
int32_t UsbHostImplMock::OpenDevice(const UsbDev &dev)
{
    if (BUS_NUM_ERR == dev.busNum && DEV_ADDR_ERR == dev.devAddr) {
        return HDF_DEV_ERR_NO_DEVICE;
    }
    return HDF_SUCCESS;
}

int32_t UsbHostImplMock::GetDeviceDescriptor(const UsbDev& dev, std::vector<uint8_t>& descriptor)
{
    return GetRawDescriptor(dev, descriptor);
}
int32_t UsbHostImplMock::GetStringDescriptor(const UsbDev &dev, uint8_t descId, std::vector<uint8_t> &decriptor)
{
    (void)descId;
...
int32_t UsbHostImplMock::BindUsbdHostSubscriber(const sptr<IUsbdSubscriber> &subscriber)
{
    subscriber_ = subscriber;
    return HDF_SUCCESS;
}

int32_t UsbHostImplMock::UnbindUsbdHostSubscriber(const sptr<IUsbdSubscriber> &subscriber)
{
    (void)subscriber;
    subscriber_ = nullptr;
    return HDF_SUCCESS;
...
int32_t UsbHostImplMock::SubscriberDeviceEvent(const USBDeviceInfo &info)
{
    auto ret = subscriber_->DeviceEvent(info);
    return ret;
}
int32_t UsbHostImplMock::GetConfig(const UsbDev &dev, uint8_t &configIndex)
{
    (void)dev;
    configIndex = 1;
    return HDF_SUCCESS;
