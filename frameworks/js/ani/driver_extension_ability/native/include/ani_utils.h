...
public:
    static ani_object Create(ani_env *env, const char *nsName, const char *clsName, ...)
    {
        ...
        if (ANI_OK != env->Class_FindMethod(cls, "<ctor>", nullptr, &ctor)) {
            HILOG_ERROR("[ANI] Not found <ctor> for class %{public}s", clsName);
            return nullobj;
        }
        ...
        va_end(args);
        if (ANI_OK != status) {
            HILOG_ERROR("[ANI] Failed to Object_New for class %{public}s", clsName);
            return nullobj;
        }
        return obj;
    }
...
public:
    ...
    static ani_string ToAni(ani_env *env, const std::string &str)
    {
        ...
        if (ANI_OK != env->String_NewUTF8(str.data(), str.size(), &aniStr)) {
            HILOG_ERROR("[ANI] Unsupported ANI_VERSION_1");
            return nullptr;
        }
        return aniStr;
    }
