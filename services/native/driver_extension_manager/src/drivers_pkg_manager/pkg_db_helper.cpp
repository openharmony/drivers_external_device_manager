...
int32_t PkgDbHelper::AddOrUpdateRightRecordEx(bool isUpdate, const std::string & bundleName, const std::string & bundleAbility, const std::string &driverInfo)
{
    int32_t ret = 0;
    ValuesBucket values;
    values.Clear();
...
int32_t PkgDbHelper::QueryAllBundleInfoNames(const std::string &bundleAbility, std::vector<std::string> &bundleInfoNames)
{
    std::lock_guard<std::mutex> guard(databaseMutex_);
    std::vector<std::string> columns = {"driverInfo"};
    RdbPredicates rdbPredicates(PKG_TABLE_NAME);
    // rdbPredicates.EqualTo("bundleAbility", bundleAbility)->Distinct();
    return QueryAndGetResultColumnValues(rdbPredicates, columns, "driverInfo", bundleInfoNames);
}

int32_t PkgDbHelper::QueryAllBundleAbilityNames(const std::string &bundleName,std::vector<std::string> &bundleAbilityNames)
{
    std::lock_guard<std::mutex> guard(databaseMutex_);
