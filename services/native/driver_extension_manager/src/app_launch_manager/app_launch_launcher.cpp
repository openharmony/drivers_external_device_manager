...
int32_t AppLaunchLauncher::LaunchApp(const std::string &bundleName, const std::string &abilityName)
{
    ...
    EDM_LOGI(MODULE_BUS_USB, "%{public}s enter, bundleName=%{public}s, abilityName=%{public}s",
    ...
    if (ret != 0) {
        EDM_LOGE(MODULE_BUS_USB, "StartAbility failed for %{public}s/%{public}s, ret=%{public}d",
            bundleName.c_str(), abilityName.c_str(), ret);
        return EDM_NOK;
    }

    EDM_LOGI(MODULE_BUS_USB, "StartAbility success for %{public}s/%{public}s",
